...
function renderBarChart(songs) {
    const max        = Math.max(...songs.map(c => c.value));
    const GRID_STEPS = 5;
...
function renderLineChart(songs) {
    ...
    const max   = Math.max(...songs.map(c => c.value)) * 1.1;
    const count = songs.length;

    const xs = songs.map((_, i) => PAD + (i / (count - 1)) * (W - PAD * 2));
    const ys = songs.map(c => H - PAD - (c.value / max) * (H - PAD * 2));
...
function renderStats(songs) {
    const values = songs.map(c => c.value);
    ...
    const maxSong = songs.find(c => c.value === max).name;
    const minSong = songs.find(c => c.value === min).name;
